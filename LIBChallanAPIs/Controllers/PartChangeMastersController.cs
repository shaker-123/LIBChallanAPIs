...
{
    [ApiController]
    [Route("api/part-change-masters")]
    [Authorize(Roles = "SUPER_ROLE,ADMIN_ROLE")]
    public class PartChangeMastersController : ControllerBase
    {
        private readonly IPartChangeMasterRepository _repository;

        public PartChangeMastersController(IPartChangeMasterRepository repository)
        {
        ...
        [HttpGet("part-number/{partNumber}")]
        public async Task<IActionResult> GetByPartNumber(string partNumber)
        {
            var result = await _repository.GetByPartNameAsync(partNumber);
            return result == null ? NotFound() : Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PartChangeMasterCreateDto dto)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] PartChangeMasterUpdateDto dto)
        {
