...
{
    [ApiController]
    [Route("api/defect-details")]
    [Authorize(Roles = "SUPER_ROLE,ADMIN_ROLE")]
    public class DefectDetailsController : ControllerBase
    {
        private readonly IDefectDetailsRepository _repository;

        public DefectDetailsController(IDefectDetailsRepository repository)
        {
        ...
        {
            var result = await _repository.GetByDefectNameAsync(code);
            return result == null ? NotFound() : Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(DefectDetailsCreateDto dto)
            => Ok(await _repository.CreateAsync(dto));

        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, DefectDetailsUpdateDto dto)
        {
