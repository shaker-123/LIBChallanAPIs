...
namespace LIBChallanAPIs.Controllers
{
    ...
    public class CorrectiveActionsController : ControllerBase
    {
        ...
        {
            var result = await _repository.GetByActionNameAsync(code);
            return result == null ? NotFound() : Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CorrectiveActionCreateDto dto)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] CorrectiveActionUpdateDto dto)
        {
