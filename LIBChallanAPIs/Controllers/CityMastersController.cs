...
{
    [ApiController]
    [Route("api/city-masters")]
    [Authorize(Roles = "SUPER_ROLE,ADMIN_ROLE")]
    public class CityMastersController : ControllerBase
    {
        private readonly ICityMasterRepository _repository;

        public CityMastersController(ICityMasterRepository repository)
        {
        ...
        [HttpGet("active/{stateId}")]
        public async Task<IActionResult> GetAllActive(string stateId)
        {
            return Ok(await _repository.GetAllActiveAsync(stateId));
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CityMasterCreateDto dto)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] CityMasterUpdateDto dto)
        {
